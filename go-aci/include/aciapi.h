...
sword ACIParamSet(void* hndlp, ub4 htype, ACIError* errhp,
    CONST void* dscp, ub4 dtyp, ub4 pos);
...
sword ACINumberIntPower(ACIError *err, const ACINumber *base, const sword exp,
                         ACINumber *result);
sword ACINumberPrec(ACIError* err, const ACINumber* number, eword nDigs,
                         ACINumber* result);
...
sword ACINlsGuessCharset(void const* buf, size_t len, ub2 *charsetid);
...
sword ACIResultSetToStmt(ACIResult* rsetdp, ACIError* errhp);
...
sword  ACIXMLTypeCreateFromSrc(ACISvcCtx* svchp, ACIError* errhp,
    ACIDuration dur, ub1 src_type, dvoid* src_ptr,
    sb4 ind, ACIXMLType** retInstance);

sword  ACISvcCtxToLda(ACISvcCtx* svchp, ACIError* errhp, Lda_Def* ldap);

sword  ACILdaToSvcCtx(ACISvcCtx* svchp, ACIError* errhp, Lda_Def* ldap);

sword  ACIContextClearValue(void *hndl, ACIError *err, ub1 *key, ub1 keylen);

sword  ACIContextGenerateKey(void *hndl,ACIError *err,ub4 *key);

sword  ACIContextGetValue(void *hndl, ACIError   *err, ub1 *key, ub1 keylen, void **ctx_value);

sword ACIContextSetValue(void *hndl, ACIError *err, ACIDuration  duration, ub1 *key, ub1 keylen, void *ctx_value);

sword ACIExtractFromFile(void *hndl,ACIError *err, ub4 flag, OraText *filename);

sword ACIExtractFromList(void *hndl, ACIError *err,
	uword index, OraText **name, ub1 *type, uword *numvals, void ***values);

sword ACIExtractFromStr(void *hndl, ACIError *err,
	ub4 flag, OraText  *input);

sword ACIExtractInit(void *hndl, ACIError *err);

sword ACIExtractReset(void *hndl, ACIError  *err);

sword ACIExtractSetKey(void *hndl, ACIError *err,
	const text  *name, ub1 type, ub4 flag, const void  *defval, const sb4   *intrange, const text  *strlist);

sword ACIExtractSetNumKeys(void *hndl, ACIError *err, uword numkeys);

sword ACIExtractTerm(void *hndl, ACIError  *err);

sword ACIExtractToBool(void *hndl, ACIError  *err,
	OraText   *keyname, uword valno, ub1 *retval);

sword ACIExtractToInt(void *hndl, ACIError  *err,
	OraText *keyname, uword valno, sb4 *retval);

sword ACIExtractToList(void *hndl, ACIError  *err, uword *numkeys);

sword ACIExtractToOCINum(void *hndl,ACIError *err,
	OraText *keyname, uword valno, ACINumber *retval);

sword ACIExtractToStr(void *hndl, ACIError *err,
	OraText *keyname, uword valno, OraText *retval, uword buflen);

sword ACIFileClose(void *hndl, ACIError *err, ACIFileObject *filep);

sword ACIFileExists(void *hndl, ACIError *err,
	OraText  *filename, OraText  *path, ub1 *flag);

sword ACIFileFlush(void *hndl, ACIError *err, ACIFileObject *filep);

sword ACIFileGetLength(void *hndl, ACIError *err,
	OraText  *filename, OraText  *path, ubig_aci *lenp);

sword ACIFileInit(void *hndl, ACIError *err);

sword ACIFileOpen(void *hndl, ACIError *err,
	ACIFileObject  **filep, OraText *filename, OraText *path, ub4 mode, ub4 create, ub4 type);

sword ACIFileRead(void *hndl, ACIError *err,
	ACIFileObject *filep, void *bufp, ub4 bufl, ub4 *bytesread);

sword ACIFileSeek(void *hndl, ACIError *err,
	ACIFileObject *filep, uword origin, ubig_aci offset, sb1 dir);

sword ACIFileTerm(void *hndl, ACIError *err);

sword ACIFileWrite(void *hndl, ACIError *err,
	ACIFileObject *filep, void *bufp, ub4 buflen, ub4 *byteswritten);

sword ACIFormatInit(void *hndl, ACIError  *err);

sword ACIFormatString(void *hndl, ACIError *err,
	OraText *buffer, sbig_aci bufferLength, sbig_aci *returnLength, const OraText *formatString, ...);

sword ACIFormatTerm(void *hndl,  ACIError *err);

void  * ACIExtProcAllocCallMemory(ACIExtProcContext *with_context,size_t amount);

sword ACIExtProcGetEnv(ACIExtProcContext *with_context, ACIEnv **envh, ACISvcCtx **svch, ACIError **errh);

size_t ACIExtProcRaiseExcp(ACIExtProcContext *with_context, int errnum);

size_t ACIExtProcRaiseExcpWithMsg(ACIExtProcContext *with_context,
	int errnum, char *errmsg, size_t msglen);

ACIExpBuffer	createOSCExpBuffer(void);
void			initOSCExpBuffer(ACIExpBuffer secbuf);
void			destroyOSCExpBuffer(ACIExpBuffer secbuf);
void			termOSCExpBuffer(ACIExpBuffer secbuf);
void			resetOSCExpBuffer(ACIExpBuffer secbuf);
int				enlargeOSCExpBuffer(ACIExpBuffer secbuf, size_t needed);
void			printfOSCExpBuffer(ACIExpBuffer secbuf, const char *fmt, ...);
void			appendOSCExpBuffer(ACIExpBuffer secbuf, const char *fmt, ...);
void			appendOSCExpBufferStr(ACIExpBuffer secbuf, const char *data);
void			appendOSCExpBufferChar(ACIExpBuffer secbuf, char oneChar);
void			appendBinaryOSCExpBuffer(ACIExpBuffer secbuf, const char *bindata, size_t datalen);
