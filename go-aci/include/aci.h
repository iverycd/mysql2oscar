...
/*---------------------------------------------------------------------------
	OSCExpBuffer导出
---------------------------------------------------------------------------*/
#ifdef USE_OSC_EXPBUFFER
typedef struct OSCExpBufferData
{
   char    *data;      //!< 数据buf
   size_t   len;      //!< dataPtr中存储的数据长度
   size_t   maxlen;      //!< dataPtr内存大小
} OSCExpBufferData, * OSCExpBuffer;
#endif

typedef struct OSCExpBufferData * ACIExpBuffer;
...
/*-----------------------------句柄类型----------------------------------*/
												/* 句柄类型范围是1-49*/
#define ACI_HTYPE_FIRST						1				/* 句柄类型的起始值*/
#define ACI_HTYPE_ENV						1              /* 环境句柄 */
#define ACI_HTYPE_ERROR						2              /* 错误句柄 */
#define ACI_HTYPE_SVCCTX					3              /* 服务句柄 */
#define ACI_HTYPE_STMT						4              /* 语句句柄 */
#define ACI_HTYPE_BIND						5              /* 绑定句柄 */
#define ACI_HTYPE_DEFINE					6              /* 定义句柄 */
#define ACI_HTYPE_DESCRIBE					7              /* 描述句柄 */
#define ACI_HTYPE_SERVER					8              /* 服务器句柄 */
#define ACI_HTYPE_SESSION					9                  /* 用户句柄 */
#define ACI_HTYPE_AUTHINFO					ACI_HTYPE_SESSION   /* 认证句柄 */
#define ACI_HTYPE_TRANS						10                  /* 事务句柄 */
...
#define ACI_HTYPE_BULKIMP					30				/* 加载句柄 */		
#define ACI_HTYPE_BULK   					31				/* 加载句柄 */
#define ACI_HTYPE_RAW   					32				/* 二进制句柄 */

#define ACI_HTYPE_LAST						32				/* 句柄的结束值 */


/*-------------------------描述类型----------------------------------*/
												/* 描述类型的值为：50 - 255 */
#define ACI_DTYPE_FIRST						50		/* 描述类型的开始值 */
#define ACI_DTYPE_LOB						50      /* lob描述符 */
#define ACI_DTYPE_SNAP						51      /* 快照描述符 */
#define ACI_DTYPE_RSET						52      /* 结果集描述符，stmt类型 */
#define ACI_DTYPE_PARAM						53		/* 一个参数描述 */
#define ACI_DTYPE_ROWID						54      /* rowid 描述符 */
#define ACI_DTYPE_COMPLEXOBJECTCOMP			55      /* complex object retrieval descriptor */						
#define ACI_DTYPE_FILE						56      /* 文件lob描述符 */
#define ACI_DTYPE_AQENQ_OPTIONS				57      /* 队列描述符 */
#define ACI_DTYPE_AQDEQ_OPTIONS				58      /* dequeue 描述符 */
...
/*--------------------------------lob 类型 ---------------------------------*/
#define ACI_TEMP_BLOB						1		/* BLOB  类型 */
#define ACI_TEMP_CLOB						2		/* CLOB  类型 */
#define ACI_TEMP_BFILE						3		/* BFILE 类型 */


/*-------------------------------对象指针类型-------------------------------*/
#define ACI_OTYPE_NAME						1		/* 对象名称 */
...
#define  ACI_DURATION_NULL					(ACIDuration)(ACI_DURATION_BEGIN-1)		/* null duration */
...
#define  ACI_DURATION_NEXT					(ACIDuration)(ACI_DURATION_BEGIN-4)		/* next special duration */
#define  ACI_DURATION_SESSION				(ACIDuration)(ACI_DURATION_BEGIN)		/* the end of user session */
#define  ACI_DURATION_TRANS					(ACIDuration)(ACI_DURATION_BEGIN+1)		/* the end of user transaction */
/******************************************************************************
**  DO NOT USE ACI_DURATION_CALL. IT  IS UNSUPPORTED                         **
**  WILL BE REMOVED/CHANGED IN A FUTURE RELEASE                              **
******************************************************************************/
#define  ACI_DURATION_CALL					(ACIDuration)(ACI_DURATION_BEGIN+2)		/* the end of user client/server call */
...
#define ACI_ATTR_CHAR_COUNT				15		/* this specifies the bind and define size in characters */
#define ACI_ATTR_PDSCL					16		/* packed decimal scale */
#define ACI_ATTR_FSPRECISION			ACI_ATTR_PDSCL   /* fs prec for datetime data types */
#define ACI_ATTR_PDPRC					17		/* packed decimal format */
#define ACI_ATTR_LFPRECISION			ACI_ATTR_PDPRC /* fs prec for datetime data types */
...
#define ACI_ATTR_ALLOC_DURATION			37		/* object cache default allocation duration */
...
#define ACI_ATTR_POSTPROCESSING_CALLBACK 40		/* Callback to process outbind data */
#define ACI_ATTR_POSTPROCESSING_CONTEXT 41		/* Callback context to process outbind data */
#define ACI_ATTR_ROWS_RETURNED			42		/* Number of rows returned in current iter - for Bind handles */
...
#define ACI_ATTR_IS_TEMPORARY			130		/* whether table is temporary */
...
#define ACI_ATTR_LIST_PKG_TYPES			137		/* parameter of the package type list */
...
#define ACI_ATTR_WALL_PWD				186		/* client wallet password */
#define ACI_ATTR_OS_AUTH				187		/* OS authentication method */
#define ACI_ATTR_WALL_CIPHERLIST		188		/* client wallet cipherlist */
...
#define ACI_ATTR_RESERVED_603                   603
#define ACI_ATTR_RESERVED_604                   604
#define ACI_ATTR_RESERVED_605                   605
#define ACI_ATTR_RESERVED_606                   606
#define ACI_ATTR_RESERVED_607                   607
#define ACI_ATTR_RESERVED_608                   608
#define ACI_ATTR_RESERVED_609                   609
#define ACI_ATTR_RESERVED_611                   611
...
#define ACI_ATTR_RESERVED_189			189
#define ACI_ATTR_DESCRIBE_NATIVE		ACI_ATTR_RESERVED_189  /* get native info via desc */
...
//数据库后台支持的text类型最大长度
#define ACI_TEXT_MAX_DATA_LEN				16777216L 
...
#define   ACI_DT_DAY_MISSING_FROM_1582	0x1000      /*  Day is one of those "missing" from 1582 */
...
#define   ACI_INTER_MINUTE_BELOW_VALID  0x200		/*Bad MiNute Low/high bit(1=low) */
#define   ACI_INTER_INVALID_SECOND      0x400		/* Bad SeCond */
#define   ACI_INTER_SECOND_BELOW_VALID  0x800		/*bad second Low/high bit(1=low) */
#define   ACI_INTER_INVALID_FRACSEC     0x1000		/* Bad Fractional second */
#define   ACI_INTER_FRACSEC_BELOW_VALID 0x2000		/* Bad fractional second Low/High */
...
#define ACI_BIND_ARRAY_ONET				0x00000200		/* bind array for onet */
/*---------------------------------------------------------------------------*/

/*-----------------------------  Various Modes ------------------------------*/
#define ACI_DEFAULT						0x00000000		/* the default value for parameters and attributes */
...
#define ACI_SESSGET_FLAGS_RAC_DATA_AFFN 0x0008			/* connected to RAC Data Affinity enabled database */
 ...
 */
#define ACI_ENV_NO_UCB					0x01		/* A user callback will not be called in ACIEnvInit() */
#define ACI_ENV_NO_MUTEX				0x08		/* the environment handle will not be protected by a mutex internally */
...
#define ACI_EXEC_ON_ONET				0x00000004		/* exec on onet */
#define ACI_STMT_SCROLLABLE_READONLY	0x00000008		/* if result set is scrollable */
...
#define ACI_SHOW_DML_WARNINGS			0x00000400		/* return ACI_SUCCESS_WITH_INFO for delete/update w/no where clause */
...
#define ACI_SYSASM						0x00008000		/* for SYSASM authorization */
#define ACI_AUTH_RESERVED_6				0x00010000		/* reserved */
#define ACI_SYSBKP						0x00020000		/* for SYSBACKUP authorization */
#define ACI_SYSDGD						0x00040000		/* for SYSDG authorization */
#define ACI_SYSKMT						0x00080000		/* for SYSKM authorization */
#define ACI_SYSRAC						0x00100000		/* for SYSRAC authorization */
...
#define ACI_TRANS_SERIALIZABLE			0x00000400		/* starts a serializable transaction */
...
#define ACI_ATTR_TYPE_NAME				8		/* name of the named data type or a package name for package private types */
#define ACI_ATTR_SCHEMA_NAME			9		/* the schema name */
#define ACI_ATTR_SUB_NAME				10      /* type name if package private type */
#define ACI_ATTR_POSITION				11		/* relative position of col/arg in the list of cols/args */
#define ACI_ATTR_PACKAGE_NAME			12		/* package name of package type */
#define ACI_ATTR_REFERENCED				13		/* column is referenced (reserved) */
#define ACI_ATTR_PRECISION2			14		/* precision if number type */
#define ACI_ATTR_SCALE2				15		/* scale if number type */
...
#define ACI_ATTR_SHOW_INVISIBLE_COLUMNS    460		/* invisible columns support */
...
#define ACI_ATTR_TYPE_ID				   303		/* column type id */
...
#define ACI_ATTR_LIST_ACTION_CONTEXT        357		/* list of action context */
...
#define ACI_ATTR_DIRPATH_STORAGE_INITIAL	140		/* initial extent size */
#define ACI_ATTR_DIRPATH_STORAGE_NEXT       141		/* next extent size */
            /* direct path index maint method (see aci8dp.h) */
...
/* server attribute */
#define ACI_ATTR_RESERVED_37				449

/*-------------------Attributes for OCI column security-----------------------*/
#define ACI_ATTR_NO_COLUMN_AUTH_WARNING		450
#define ACI_ATTR_XDS_POLICY_STATUS			451

/* --------------- ip address attribute in environment handle -------------- */
#define ACI_ATTR_SUBSCR_IPADDR				452		/* ip address to listen on  */
#define ACI_ATTR_RESERVED_40				453  
#define ACI_ATTR_RESERVED_41				454
#define ACI_ATTR_RESERVED_42				455
#define ACI_ATTR_RESERVED_43				456

/* ------------- round trip callback attributes in the process  handle ----- */
#define ACI_ATTR_RESERVED_458				458		/* reserved */
#define ACI_ATTR_RESERVED_459				459		/* reserved */

/* support at most once transaction semantics */
#define ACI_ATTR_LTXID                     462		/* logical transaction id */

#define ACI_ATTR_LAST_LOGON_TIME_UTC       463		/* Last Successful Logon Time */

#define ACI_ATTR_RESERVED_464              464
#define ACI_ATTR_RESERVED_465              465
#define ACI_ATTR_TRANSACTIONAL_TAF         466
#define ACI_ATTR_RESERVED_467              467

/* SQL translation profile session attribute */
#define ACI_ATTR_SQL_TRANSLATION_PROFILE   468

/* session handle attribute */
#define ACI_ATTR_MAX_OPEN_CURSORS          471
...
* is not.
*/
#define ACI_ATTR_ERROR_IS_RECOVERABLE      472

/* ONS specific private attribute  */
#define ACI_ATTR_RESERVED_473              473 

/* Attribute to check if ILM Write Access Tracking is enabled or not */
#define ACI_ATTR_ILM_TRACK_WRITE           474

/* Notification subscription failure callback and context */
#define	ACI_ATTR_SUBSCR_FAILURE_CBK        477
#define ACI_ATTR_SUBSCR_FAILURE_CTX        478

/* AQ JMS properties attributes */
#define ACI_ATTR_AQJMS_HEADER_PROP     479
#define ACI_ATTR_AQJMS_USER_PROP       480
#define ACI_ATTR_AQJMS_PROPERTIES      481
#define ACI_ATTR_AQJMS_MSG_TYPE        482

/* A SQL translation profile with FOREIGN_SQL_SYNTAX attribute is set in the
* database session.
*/
#define ACI_ATTR_TRANS_PROFILE_FOREIGN     483
...
#define	ACI_ATTR_DBOP                      485

/* FAN-HA private attribute */
#define ACI_ATTR_RESERVED_486              486

/* reserved */
#define ACI_ATTR_RESERVED_487              487

#define ACI_ATTR_RESERVED_488              488

#define ACI_ATTR_VARTYPE_MAXLEN_COMPAT     489

/* Max Lifetime for session */
#define ACI_ATTR_SPOOL_MAX_LIFETIME_SESSION 490

#define ACI_ATTR_RESERVED_491              491
...
//ERROR句柄相关属性
#define ACI_ATTR_INTERNAL_ERROR             1109		/* get error code for server */
...
#define ACI_ATTR_EMPTYSTRING_IS_NULL		1205
...
typedef struct ACIAQNotify				ACIAQNotify;			/* ACIAQNotify */
typedef struct ACIserverDNs				ACIserverDNs;			/* ACIserverDNs */
...
typedef struct ACIComplexObjectComp		ACIComplexObjectComp;	/* ACI COR descriptor */
...
typedef struct ACIThreadContext			ACIThreadContext;		/* ACIThread Context */
typedef struct ACIThreadMutex			ACIThreadMutex;			/* ACIThread Mutual Exclusion Lock */
typedef struct ACIThreadKey				ACIThreadKey;			/* ACIThread Key for Thread-Specific Data */
typedef struct ACIThreadId				ACIThreadId;			/* ACIThread Thread ID */
typedef struct ACIThreadHandle			ACIThreadHandle;		/* ACIThread Thread Handle */
...
typedef struct ACISodaOperationOptions	ACISodaOperationOptions;	/*OCI SODA document cursor */
typedef struct ACISodaOutputOptions		ACISodaOutputOptions;		/* OCI SODA output optns */
typedef struct ACISodaMetadata			ACISodaMetadata;			/* OCI SODA metadata of a collection*/
typedef struct ACISodaCollCursor		ACISodaCollCursor;			/* OCI SODA collection cursor */
...
/*---------------------------------------------------------------------------*/
typedef struct ACIFileObject			ACIFileObject;				/* Represents an open file */
...
*/
typedef struct ACIExtProcContext		ACIExtProcContext;
...
#define SQLT_BULK_CHR			1			/* for All numerical, string, date/time/interval, BIT, CLOB */
#define SQLT_BULK_BIN2BIN		2			/* for BINARY、VARBINARY (bin convert to hex) */
#define SQLT_BULK_BIN2BLOB	    3			/* for BLOB (no convert) */
#define SQLT_BULK_HEX2BIN		4			/* for BINARY、VARBINARY (no convert) */
#define SQLT_BULK_HEX2BLOB		5			/* for BLOB (hex convert to bin) */
...
#define ACI_TYPECODE_DECIMAL			SQLT_PDN			/* SQL DECIMAL(P S)  OTS DECIMAL(P S) */
#define ACI_TYPECODE_UNSIGNED8			SQLT_BIN			/* SQL UNSIGNED INTEGER(8)  OTS UINT8 */
...
#define ACI_TYPECODE_VARCHAR2			SQLT_VCS			 /* SQL VARCHAR2(N)  OTS SQL_VARCHAR2(N) */
#define ACI_TYPECODE_CHAR				SQLT_AFC			/* SQL CHAR(N)  OTS SQL_CHAR(N) */
#define ACI_TYPECODE_VARCHAR			SQLT_CHR			/* SQL VARCHAR(N)  OTS SQL_VARCHAR(N) */
...
#define ACI_TYPECODE_NAMEDCOLLECTION	SQLT_NCO			/* SQL/OTS NAMED COLLECTION TYPE */
...
#ifndef ORA_OCIDATE_TYPE_H
#define ORA_OCIDATE_TYPE_H
...
#ifndef ORA_OCITIME_TYPE_H
#define ORA_OCITIME_TYPE_H
/*****************************************************************************/
/*                             ORACLE DATE TYPE                              */
/*****************************************************************************/
