...
// ParseDSN parses a DSN used to connect to ShenTong
//
// It expects to receive a string in the form:
//
// [username/[password]@]host[:port][/service_name][?param1=value1&...&paramN=valueN]
//
// Connection timeout can be set in the ShenTong files: sqlnet.ora as SQLNET.OUTBOUND_CONNECT_TIMEOUT or tnsnames.ora as CONNECT_TIMEOUT
//
...
func ParseDSN(dsnString string) (dsn *DSN, err error) {
	...
	for k, v := range qp {
		...
		case "as":
			switch v[0] {
			case "SYSDBA", "sysdba":
				dsn.operationMode = C.ACI_DEFAULT
			case "SYSASM", "sysasm":
				dsn.operationMode = C.ACI_DEFAULT
...
func (drv *DriverStruct) Open(dsnString string) (driver.Conn, error) {
	...
	result = C.OCIHandleAlloc(
		...
		C.OCI_HTYPE_ERROR,        // type of handle
