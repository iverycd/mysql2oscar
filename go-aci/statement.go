...
func (stmt *Stmt) bindValues(ctx context.Context, values []driver.Value, namedValues []driver.NamedValue) ([]bindStruct, error) {
	...
	for i := 0; i < count; i++ {
		...
		*sbind.indicator = 0
			...
			if isOut {
				...
				if len(value) > 32767 {
					...
					*sbind.length = C.ub2(sizeOfNilPointer)
					...
					sbind.dataType = C.SQLT_AFC
					if len(value) == 0 {
						sbind.pbuf = unsafe.Pointer(C.CString(""))
						sbind.maxSize = C.sb4(1)
						*sbind.length = C.ub2(1)
					} else {
		...
		case bool: // ShenTong does not have bool, handle as 0/1 int
			sbind.dataType = C.SQLT_INT
			if value {
...
func (stmt *Stmt) makeDefines(ctx context.Context) ([]defineStruct, error) {
	...
	for i := 0; i < paramCount; i++ {
		...
		var maxSize C.ub8 // Maximum size in bytes of the external data for the column. This can affect conversion buffer sizes.
		...
		defines[i].length = (*C.ub4)(C.malloc(C.sizeof_ub2))
		*defines[i].length = 0
		defines[i].indicator = (*C.sb2)(C.malloc(C.sizeof_sb2))
		*defines[i].indicator = 0
		// 获取字段类型名
		var typename *C.OraText
		var typename_size C.ub4
		typename_size, err = stmt.conn.ociAttrGet(param, unsafe.Pointer(&typename), C.OCI_ATTR_TYPE_NAME)
		if err != nil {
			freeDefines(defines)
			return nil, err
		}
		defines[i].dataTypeName = cGoStringN(typename, int(typename_size))
		...
		case C.SQLT_AFC, C.SQLT_CHR, C.SQLT_VCS, C.SQLT_AVC:
			...
			defines[i].maxSize = C.sb8(maxSize * 2)
			defines[i].pbuf = C.malloc(C.size_t(defines[i].maxSize))
			//defines[i].length = (*C.ub2)(defines[i].maxSize)

		case C.SQLT_BIN:
			defines[i].dataType = C.SQLT_BIN
			defines[i].maxSize = C.sb8(maxSize)
			defines[i].pbuf = C.malloc(C.size_t(defines[i].maxSize))

		case C.SQLT_LBI:
			defines[i].dataType = C.SQLT_LBI
			defines[i].maxSize = C.sb8(65535)
			defines[i].pbuf = C.malloc(C.size_t(defines[i].maxSize))
		...
		case C.SQLT_CLOB, C.SQLT_BLOB:
			...
			defines[i].maxSize = C.sb8(sizeOfNilPointer)
			var lobP *unsafe.Pointer
			lobP, _, err = stmt.conn.ociDescriptorAlloc(C.OCI_DTYPE_LOB, 0)
			if err != nil {
				freeDefines(defines)
...
func (stmt *Stmt) getRowid() (string, error) {
	...
	result := C.ACILastInsertId(stmt.stmt, rowid, &rowidLength)
	if result != C.OCI_SUCCESS {
